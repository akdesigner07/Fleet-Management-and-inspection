...
const ConsortiumClearinghouse = () => {
  ...
  const fetchQueries = async () => {
    try {
      setLoading(true);
      const params = new URLSearchParams({ type: 'clearinghouse_query', limit: '50' });
      ...
      console.error('Failed to load clearinghouse queries:', err);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div>
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          <div>
            ...
            <p style={{ fontSize: '13.5px', color: '#94a3b8', margin: '2px 0 0 0' }}>
              Commercial Driver Drug &amp; Alcohol Clearinghouse verification requests
            </p>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', gap: '10px' }}>
          ...
          >
            <Plus size={16} />
            <span>Order Clearinghouse Query</span>
          </Link>
          <button onClick={fetchQueries} className="btn-consortium-secondary">
            <RefreshCw size={15} />
          </button>
        </div>
      </div>
        ...
        <form onSubmit={handleSearch} className="consortium-filters-bar" style={{ marginBottom: 0 }}>
          <div style={{ position: 'relative', flex: 1, minWidth: '240px' }}>
            ...
            <input
              ...
              placeholder="Search by query ID, driver name, company, or CDL..."
      ...
      <div className="consortium-card">
        {loading ? (
          <div style={{ textAlign: 'center', padding: '50px 20px', color: '#94a3b8' }}>
            ...
            <span>Loading Clearinghouse queries...</span>
          </div>
        ) : requests.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '50px 20px', color: '#94a3b8' }}>
            <Database size={36} style={{ color: '#475569', marginBottom: '12px' }} />
            <p style={{ fontSize: '15px', color: '#cbd5e1' }}>No Clearinghouse queries found.</p>
            <Link to="/consortium_panel/clearinghouse/create" className="btn-consortium-primary" style={{ marginTop: '12px' }}>
              Create First Clearinghouse Query
            </Link>
          </div>
        ) : (
                  ...
                  <tr key={r.id}>
                    ...
                    <td>
                      <span style={{
                        ...
                        fontSize: '11.5px',
                        fontWeight: 700,
                        background: r.ch_result?.includes('No Violation') ? 'rgba(16, 185, 129, 0.15)' : 'rgba(100, 116, 139, 0.15)',
                        color: r.ch_result?.includes('No Violation') ? '#34d399' : '#94a3b8'
                      }}>
                        {r.ch_result || 'Pending Query'}
                      </span>
                    </td>
                    <td>
...
export default ConsortiumClearinghouse;
